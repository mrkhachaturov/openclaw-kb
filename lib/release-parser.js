...
const MAX_APPCAST_NOTES_CHARS = 1500;

export function getReleaseFamily(tag) {
  if (!tag) return tag;
  return tag
    .replace(/-beta\.\d+$/i, '')
    .replace(/-\d+$/, '');
...
export function selectReleaseWindow(releases, familyLimit = 3) {
  const familyOrder = [];
  const selectedFamilies = new Set();
  const grouped = new Map();

  for (const release of releases) {
    const family = getReleaseFamily(release.tag);
    if (!grouped.has(family)) grouped.set(family, []);
    grouped.get(family).push(release);
  ...
  return familyOrder
    .slice(0, familyLimit)
    .flatMap(family => grouped.get(family) || []);
}
...
function getTagInfo(tag, upstreamDir) {
  try {
    // git log -1 dereferences annotated/signed tags to the underlying commit
    // and outputs only the format line (unlike `git show <tag>` which prints
    // the tag header + signature first for annotated tags, breaking split).
    const cmd = `git log -1 --format='%H|%aI|%an <%ae>' ${tag}`;
...
export function formatChangelogMarkdown(metadata) {
  ...
  if (metadata.appcast_notes) {
    md += `\n## Release Notes\n\n`;
    const notes = metadata.appcast_notes.length > MAX_APPCAST_NOTES_CHARS
      ? `${metadata.appcast_notes.slice(0, MAX_APPCAST_NOTES_CHARS)}...`
      : metadata.appcast_notes;
    md += notes + '\n';
  }
...
function renderCategory(title, items = [], summaryLabel, maxItems = MAX_RELEASE_CATEGORY_ITEMS) {
  ...
  if (items.length > maxItems) {
    section += `- ... and ${items.length - maxItems} more ${summaryLabel}\n`;
  }
  section += '\n';
  return section;
}
