...
export function getChunksSinceRelease(sinceRelease, limit = 100) {
  const release = getReleaseByTag(sinceRelease);

  let rows;
  if (release) {
    rows = prepare(`
      SELECT c.id, c.path, c.source, c.content_type, c.language, c.category, c.indexed_release,
             c.start_line, c.end_line, c.text, r.date as release_date
      FROM chunks c
      JOIN releases r ON r.tag = c.indexed_release
      WHERE r.date >= ?
      ORDER BY r.date DESC, c.path ASC
      LIMIT ?
    `).all(release.date, limit);
  } else {
    rows = prepare(`
...
export function getCurrentIndexedRelease() {
  const row = prepare(`
    SELECT r.*
    FROM chunks c
    LEFT JOIN releases r ON r.tag = c.indexed_release
    WHERE c.indexed_release IS NOT NULL
    GROUP BY c.indexed_release
    ORDER BY COUNT(*) DESC, COALESCE(r.date, c.indexed_release) DESC
    LIMIT 1
  `).get();
